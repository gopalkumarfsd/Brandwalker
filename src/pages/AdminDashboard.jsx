  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (!subscriberResponse.ok) {
        throw new Error(
          subscriberData.message || "Unable to fetch subscribers"
        );
      }

      if (!inquiryResponse.ok) {
        throw new Error(
          inquiryData.message || "Unable to fetch inquiries"
        );
      }
  ...
  const handleDeleteInquiry = async (id) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          data.message || "Unable to delete inquiry"
        );
      }
      ...
      setError(
        error.message || "Unable to delete inquiry"
      );
    } finally {
      setDeletingId("");
    }
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Recent inquiries submitted from the website.
                </p>
              </div>
